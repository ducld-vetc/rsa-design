...
| Dialog — Preference | shift_tab | Tabs KEY | String | Y | Preference theo KEY | C1, C3, CG… |
...
| Dialog — Preference | day_mode | Toggle | Enum | Y | Một ngày một loại | **Ngày nghỉ** / **Muốn đi làm** |
| Dialog — Preference | calendar | Month grid | Number[] | N | Click toggle ngày | offDays / preferredWorkDays |
...
| UC-05 | Sắp xếp ca mặc định | Auto | Admin | Mode Cập nhật | 1. **Sắp xếp ca mặc định**<br>2. Xem kết quả → **Áp dụng**<br>3. Unlock Action 2 | Draft FULL mới | BR-10–13 |
| UC-06 | Bổ sung ca gãy | Auto | Admin | Đã áp Action 1 | 1. **Bổ sung ca gãy**<br>2. Áp dụng → draft | SPLIT bù coverage | BR-14, 15 |
| UC-07 | Cấu hình mong muốn theo ca | Pref | Admin | Mode Cập nhật | 1. **Mong muốn theo ca**<br>2. Chọn KEY + NV + ngày nghỉ/ưu tiên<br>3. **Xong** | Pref lưu context ngay | BR-16 |
...
| **BR-10** | Action 1: **xóa hết** assignment ngày → xếp FULL (đêm trước, rồi `minStaff` cao). |
...
| **BR-15** | Action 2: giữ FULL; bù SPLIT theo thiếu coverage; C3 không có fallback split. |
| **BR-16** | Preference: `offDays` chặn auto; `preferredWorkDays` tăng điểm; một ngày không vừa nghỉ vừa ưu tiên. |

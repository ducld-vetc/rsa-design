...
const PARTNER_SERVICES = ALL_SERVICES.filter(s => s.provider !== 'VETC');
...
const AVAILABLE_SERVICES = PARTNER_SERVICES.map(s => s.name);
...
const StationCreateOrder: React.FC = () => {
  const navigate = useNavigate();
  const rescueOrderId: string | null = null;
  const [activeTab, setActiveTab] = useState('general');

  const [selectedServices, setSelectedServices] = useState<string[]>(['Xe hết pin', 'Đâm, lật, tai nạn']);
  ...
  const [selectedPackage, setSelectedPackage] = useState('Không có');
  ...
  const [mapAddress, setMapAddress] = useState("20, Phố Hội Vũ, Khu phố cổ, Phường Hoàn Kiếm, Hà Nội");
  const [mapCoords, setMapCoords] = useState("21.0285, 105.8452");
  ...
  const [totalPayment, setTotalPayment] = useState('432,000');
  ...
  // Tabs Configuration
  const TABS = [
    { id: 'general', label: 'Thông tin sự cố', icon: <User size={16}/>, hideInCreate: false },
    { id: 'orchestration', label: 'Điều phối cứu hộ', icon: <LifeBuoy size={16}/>, hideInCreate: false },
    { id: 'process', label: 'Quá trình cứu hộ', icon: <Activity size={16}/>, hideInCreate: true },
    { id: 'images', label: 'Hình ảnh', icon: <Camera size={16}/>, hideInCreate: false },
    { id: 'monitoring', label: 'Giám sát & Thực thi', icon: <ShieldCheck size={16}/>, hideInCreate: true },
  ];
      ...
      <div className="space-y-6 animate-in fade-in duration-700 max-w-[1600px] mx-auto pb-20">
        ...
        {rescueOrderId && (
            <div className="sticky top-0 z-20 flex flex-col bg-white/95 backdrop-blur-md border rounded-xl shadow-md border-l-4 border-l-vetc-green text-left">
              {/* Tab Navigation Bar */}
              <div className="flex items-center px-4 bg-gray-50/50 overflow-x-auto custom-scrollbar rounded-t-xl">
                {TABS.map(tab => {
                  return (tab.hideInCreate && !rescueOrderId) ? <></> : (
                      <button
                          key={tab.id}
        ...
        <div className="grid grid-cols-1 gap-6 text-left">
            ...
            <div className="border rounded-lg shadow-sm bg-white overflow-visible">
              ...
              <div className="p-5 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-4">
                ...
                <div>
                  ...
                  <Input defaultValue="29E366666" readOnly={!isEditing} />
                </div>
                <div>
              ...
              <div className="p-5 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-4 pt-4 border-t border-gray-50 text-left">
                  ...
                  <div className="lg:col-span-2">
                    ...
                    <div className="flex space-x-2">
                      <Input defaultValue="Phường Việt Hưng, Quận Long Biên, Hà Nội" className="flex-1" readOnly={!isEditing} />
                      <button
                          onClick={() => setIsMapModalOpen(true)}
                          disabled={!isEditing}
                          className={`bg-vetc-green text-white px-4 py-1.5 rounded text-[11px] font-bold shrink-0 hover:bg-green-700 transition-all flex items-center space-x-2 active:scale-95 shadow-sm ${!isEditing ? 'opacity-50 cursor-not-allowed' : ''}`}
          ...
          {/* 4. Hình ảnh sự cố (Renumbered from 5) */}
          <div id="section-images" className={`scroll-mt-40 block`}>
            <div className="border rounded-lg shadow-sm bg-white overflow-hidden">
              <SectionHeader title="Hiện trường sự cố" number={4} icon={<Camera size={18} />} />
              <div className="p-6">
                <ImageUploadSection readOnly={!isEditing} onlyScene />
              </div>
            </div>
          </div>

          {/* 5. Thông tin giám sát, thực thi (Renumbered from 6) */}
          <div id="section-monitoring" className={`scroll-mt-40 ${rescueOrderId ? 'block' : 'hidden'}`}>
        ...
        <div className="sticky bottom-[-24px] z-40 bg-white/95 backdrop-blur-md pt-4 pb-6 px-6 -mx-6 border-t flex justify-between items-center shadow-[0_-10px_15px_-3px_rgba(0,0,0,0.05)]">
          ...
          <div className="flex items-center space-x-3">
            <button
                ...
                className="bg-vetc-green text-white px-14 py-3 rounded-xl font-bold flex items-center space-x-2 shadow-lg hover:bg-green-700 active:scale-95 transition-all uppercase tracking-wide group"
            >
...
export default StationCreateOrder;
